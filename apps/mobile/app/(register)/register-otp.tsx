...
export default function EmailScreen () {
    ...
    const handleResend = async () => {
        ...
        try {
            const response = await fetch(`${config.apiBaseUrl}/auth/email-otp/send-verification-otp`, {
                ...
                body: JSON.stringify({ email: registerData.email, type: 'email-verification' })
            });
            try {
                if (response.ok) {
    ...
    const handleRegister = async () => {
        ...
        try {
          ...
          if (response.ok) {
            ...
            console.log("Request sent", registerData.email, registerData.password)
            if (loginResponse.ok) {
    ...
    return (
        <View style={styles.container}>
            ...
            <Text style={styles.subtext}>Enter the 6 digit code sent to {registerData.email}.</Text>
